...
- `server.js`（仓库根）— **V1 Express 遗留服务器**，生产 PM2 实际仍运行此入口，使用本地 `exports/` 目录存照片。
- `src/app/` — **V2 Next.js App Router**，所有新功能与 API 的开发入口，使用阿里云 OSS 前端直传。

新 API 一律写到 `src/app/api/`，**不要往 `server.js` 里加代码**。两套服务共享同一 SQLite Schema 但 V1 的 `db.js`（根）无日志/链路追踪集成。
...
- **部署**：GitHub Actions self-hosted runner（`.github/workflows/deploy.yml`），push 到 `main` 触发：`actions/setup-node@v4`(Node 22) → `pnpm/action-setup@v4` → `pnpm install --frozen-lockfile` → `pnpm build`（Linux 下 `next.config.mjs` 输出 standalone）→ rsync 到 `/var/www/WeldSnap/`，排除 `data/` / `exports/` / `node_modules/` / `.git` / `.github` / `config.json` / `.next/cache`。
...
- **PM2 入口**：`pm2 start .next/standalone/server.js --name weld-photo-system --interpreter /opt/node-v22/bin/node --node-args="--experimental-sqlite"`（**不是**根目录 V1 `server.js`）。`pm2 delete` 先清后建，依赖 `RUNNER_TRACKING_ID` 环境变量（值为任意非空串）防止 GitHub Actions runner 完成后连带杀掉 PM2 守护的 Node 后台进程——删该变量会导致服务部署完即被杀。
